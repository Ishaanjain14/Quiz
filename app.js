...
const hasStudentSubmitted = (rollNumber) => {
  ...
  return results.some((result) => String(result.RollNumber) === String(rollNumber));
};
...
app.get("/check-submission/:rollNumber", (req, res) => {
  const { rollNumber } = req.params;
  const submitted = hasStudentSubmitted(rollNumber);
  res.json({ submitted });
});
